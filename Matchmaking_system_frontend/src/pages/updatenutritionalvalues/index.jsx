...
const UserNutritionUpdateForm = () => {
    ...
    if (formData.commonly_used_ingredients.length < 5) {
      ...
      try {
        const response = await axios.put(
          `${import.meta.env.VITE_BACKEND_BASE_API_URL}/user/nutrition`,
          submitdata,
          {
            headers: {
        ...
        alert("Your Preferences has being updated");
        ...
        console.log("Success :", clusterResponse.data.message);
      } catch (error) {
    ...
    const fetchPreferences = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_BASE_API_URL}/user/preferences/${
            user.user_id
          }`
        );
        if (response.status === 200) {
          const data = response.data; // Debugging line to see what data is fetched
          setFormData({
            ...formData,
            calories: data.calories.toString(),
            commonly_used_ingredients: data.commonly_used_ingredients
              .split(",")
              .map((item) => item.trim()), // Handle any space issues
            preferred_cuisines: data.preferred_cuisines
              .split(",")
              .map((item) => item.trim()), // As above
            allergies: data.allergies.split(",").map((item) => item.trim()),
            preferred_cooking_time: data.preferred_cooking_time || "",
            preferred_prep_time: data.preferred_prep_time || "",
            preferred_total_cooking_time:
              data.preferred_total_cooking_time || "",
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Update Your Preferences</h1>
...
export default UserNutritionUpdateForm;
